...
public class MirFillPdfFormServlet extends MCRServlet {
    ...
    private static final Logger LOGGER =
        LogManager.getLogger(MirFillPdfFormServlet.class);
    ...
    public void doGetPost(final MCRServletJob job) throws Exception {
        ...
        org.jdom2.Document xfdf =
            (org.jdom2.Document) req.getAttribute("MCRXEditorSubmission");

        if (xfdf == null) {
            throw new IllegalArgumentException(
                "No XFDF submission found (MCRXEditorSubmission missing)."
            );
        }

        String pdfPath =
            MCRFrontendUtil.getBaseURL()
            + "pdfform/Einverstaendniserklaerung.pdf";

        /*try (
            InputStream input = new URL(pdfPath).openStream();
            PDDocument pdfDocument =
                Loader.loadPDF(input.readAllBytes())
        ) {
            ...
            res.setHeader(
                "Content-Disposition",
                "attachment; filename=\"Einverstaendniserklaerung.pdf\""
            );
    ...
    private void fillPdfForm(
        ...
        if (xfa != null) {
            LOGGER.warn(
                "PDF document contains XFA - XFA is not supported."
            );
        }
        ...
        XPathExpression<Element> expr =
            xFactory.compile("//field", Filters.element());
